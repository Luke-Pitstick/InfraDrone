...
def observation(image, mask, location):
    ...
    return FrameResult(VideoFrame(image, 0, 0), location, [damage])
...
def main():
    ...
    for i, path in enumerate(selected):
        ...
        association = engine.process(video, observation(image, mask, location))[0]
    ...
    for mode in ('identity', 'view', 'view_lighting', 'wrong_place'):
        for i, item in enumerate(items):
            ...
            t = time.monotonic()
            association = engine.process(video, observation(current, current_mask, location))[0]
            expected = item['baseline']['defect_id']
            if association.defect_id is None:
                outcome = 'unresolved'
